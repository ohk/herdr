...
The hook reports Muse session identity to Herdr for native restore. State stays with Herdr's screen manifest plus OSC detection: Muse emits no Interrupt hook event and synthesizes no Stop on cancel, so lifecycle reporting would stick on working after every Esc.
