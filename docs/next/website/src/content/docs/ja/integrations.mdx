...
このフックは、ネイティブ復元のために Muse のセッション識別を Herdr に報告します。状態は Herdr のスクリーン検出と OSC 検出に残ります。Muse は Interrupt フックイベントを発行せず、キャンセル時に Stop を合成しないため、ライフサイクル報告では Esc のたびに状態が working のまま残ってしまうからです。
